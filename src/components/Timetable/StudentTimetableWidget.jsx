...
const StudentTimetableWidget = ({ studentData = {} }) => {
  const classId = useMemo(() => studentData.classId || {
    _id: "6994c0b591c15214ce95ad6b",
    name: "Class 2"
  }, [studentData.classId]);

  const classIdValue = useMemo(() => classId._id || classId, [classId]);
  ...
  const fetchTodaySchedule = async () => {
    try {
      ...
      setNextPeriod(response.data.nextPeriod);
    } catch (error) {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      {/* Header */}
      <div className="border-b border-stroke px-6 py-4 dark:border-strokedark bg-gradient-to-r from-primary to-blue-600">
      ...
      {(currentPeriod || nextPeriod) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6 bg-gray-50 dark:bg-meta-4">
          {/* Current Period */}
          <div className={`p-4 rounded-lg border-2 ${
            ...
            <div className="flex items-center gap-2 mb-3">
              <div className={`w-3 h-3 rounded-full ${currentPeriod ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`}></div>
            ...
            {currentPeriod ? (
              <>
                <h4 className="text-xl font-bold text-black dark:text-white mb-2 flex items-center gap-2">
                  ...
                  {currentPeriod.subjectId?.name}
                </h4>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                    <FiUser size={14} />
                    <span>{currentPeriod.teacherId?.firstName} {currentPeriod.teacherId?.lastName}</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                    <FiClock size={14} />
                    <span>{formatTimeSlot(currentPeriod.timeSlotId)}</span>
                  </div>
                  {currentPeriod.room && (
                    <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                      <FiMapPin size={14} />
          ...
          <div className="p-4 rounded-lg border-2 bg-blue-50 border-blue-500 dark:bg-blue-900 dark:bg-opacity-20 dark:border-blue-600">
            ...
            {nextPeriod ? (
              <>
                <h4 className="text-xl font-bold text-black dark:text-white mb-2 flex items-center gap-2">
                  <FiBook className="text-primary" />
                  {nextPeriod.subjectId?.name}
                </h4>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                    <FiUser size={14} />
                    <span>{nextPeriod.teacherId?.firstName} {nextPeriod.teacherId?.lastName}</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                    <FiClock size={14} />
                    <span>{formatTimeSlot(nextPeriod.timeSlotId)}</span>
                  </div>
                  {nextPeriod.room && (
                    <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                      <FiMapPin size={14} />
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        {entry.timeSlotId?.isBreak ? (
                          <span className="px-3 py-1 bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200 text-xs font-semibold rounded-full">
                            BREAK
                          </span>
                        ) : (
                          <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm text-gray-600 dark:text-gray-400">
                        {!entry.timeSlotId?.isBreak && (
                          <div className="flex items-center gap-1">
                            <FiUser size={12} />
                            <span>{entry.teacherId?.firstName} {entry.teacherId?.lastName}</span>
                          </div>
                        )}
                        <div className="flex items-center gap-1">
                          <FiClock size={12} />
...
export default React.memo(StudentTimetableWidget);